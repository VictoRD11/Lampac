...
{
    public class UakinoClub : BaseOnlineController
    {
        [HttpGet]
        [Route("lite/uakinoclub")]
        ...
        {
            var init = await loadKit(AppInit.conf.UakinoClub);
            ...
            var oninvk = new UakinoClubInvoke
            (
               host,
               init.corsHost(),
            ...
            var cache = await InvokeCache<EmbedModel>($"uakinoclub:view:{title}:{year}:{href}:{clarification}", cacheTime(40, init: init), rch.enable ? null : proxyManager, async res =>
            {
            ...
            return OnResult(cache, () => oninvk.Html(cache.Value, clarification, title, original_title, year, t, s, href, init.vast, rjson), gbcache: !rch.enable);
        }

        [HttpGet]
        [Route("lite/uakinoclub/video")]
        async public Task<ActionResult> Video(string link)
        {
            var init = await loadKit(AppInit.conf.UakinoClub);
            if (await IsBadInitialization(init, rch: true))
                return badInitMsg;
            ...
            string streamUrl = HostStreamProxy(init, link, proxy: proxy);
